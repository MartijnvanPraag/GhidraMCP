# GhidraMCP Ghidra Plugin HTTP API v2
...
Manage named constant values (equates) and their assignment to instruction operands. Supports multiple equates per operand and optional auto-assignment upon creation.

### Summary

| Endpoint | Methods | Description |
|----------|---------|-------------|
| `/equates` | GET | List equate definitions (with filtering & pagination) |
| `/equates` | POST/PUT | Create (idempotent) equate; optional auto-assign if `address` & `operandIndex` supplied |
| `/equates/{name}` | GET | Get details of a specific equate |
| `/equates/{name}` | DELETE | Delete equate (fails if in use unless `force=true`) |
| `/equates/{name}/usages` | GET | List usage references (address + operandIndex) |
| `/equates/assign` | POST | Assign existing or new equate to instruction operand (creates if `value` given) |
| `/equates/assign` | DELETE | Remove specific equate assignment from operand |
| `/equates/at/{address}` | GET | List all operand slots at address with equate names (filter by `operandIndex`) |
| `/equates/value/{value}` | GET | Reverse lookup: list equates matching numeric value |
...
POST /equates
{
  "name": "MY_CONST",
  "value": "0x1234",
  "address": "00401000",        // optional (auto-assign)
  "operandIndex": 1               // optional (requires address)
}
```
...
```json
{
  ...
  "_links": {
    "self": { "href": "/equates/MY_CONST" },
    "assign": { "href": "/equates/assign" },
    "usages": { "href": "/equates/MY_CONST/usages" }
  }
}
```
...
`GET /equates?offset=0&limit=50&nameContains=ERR&value=0x10`

Response `result` is an array of summary objects:
```json
{
  "success": true,
  "result": [
    {
      ...
      "valueHex": "0x0",
      "uses": 5,
      "_links": { "self": { "href": "/equates/ERR_NONE" }, "usages": { "href": "/equates/ERR_NONE/usages" } }
    }
  ],
  "size": 12,
  "offset": 0,
  "limit": 50,
  "_links": { "self": { "href": "/equates?offset=0&limit=50" } }
}
```
...
```
If the equate does not exist, include `value` to create and assign in one request.
...
```json
{
  ...
  "_links": {
    "self": { "href": "/equates/assign" },
    "equate": { "href": "/equates/ERR_NONE" },
    "at_address": { "href": "/equates/at/00401000" }
  }
}
```

### List Equates at Address

`GET /equates/at/00401000`
```json
{
  "success": true,
  "result": [
    { "address": "00401000", "operandIndex": 0, "names": ["ERR_NONE", "ALT_ERR_NONE"], "_links": { "instruction": { "href": "/address/00401000" } } }
  ],
  "_links": { "self": { "href": "/equates/at/00401000" } }
}
```

### Remove Assignment

`DELETE /equates/assign`
```json
{ "name": "ERR_NONE", "address": "00401000", "operandIndex": 0 }
```

### Delete Equate

`DELETE /equates/MY_CONST` → 409 if in use.
`DELETE /equates/MY_CONST?force=true` → removes all usages then deletes.

### Reverse Lookup by Value

`GET /equates/value/0x1234` → list all equates with that value.

### Error Codes

| Code | Meaning |
|------|---------|
| `NO_PROGRAM_LOADED` | No program context available |
| `MISSING_PARAMETER` | Required field omitted |
| `INVALID_ADDRESS` | Address parsing failed |
| `INVALID_PARAMETER` | Operand index or value malformed |
| `EQUATE_NOT_FOUND` | Named equate absent |
| `EQUATE_VALUE_CONFLICT` | Name exists with different value |
| `EQUATE_IN_USE` | Delete attempted while usages present (without force) |
| `ASSIGNMENT_NOT_FOUND` | Removal target not found |

### Notes

- Multiple equates per operand are preserved; assignment does not replace existing.
- Auto-assignment during creation is optional by supplying `address` + `operandIndex`.
- Values accept decimal or `0x` prefixed hex (case-insensitive).
- Pagination currently simple; sorting future enhancement.
...
### `GET /instances`
Returns information about all active GhidraMCP plugin instances.
```json
{
  "id": "req-instances",
  "instance": "http://localhost:8192",
