# Contributing to GhidraMCP

Thank you for your interest in contributing to GhidraMCP! This document provides guidelines and information for contributors.
...
GhidraMCP consists of two main components:

1. **Java Plugin for Ghidra** (`src/main/java/au/federation/ghidra/`):
   - Main class: `GhidraMCPPlugin.java`
...
git clone https://github.com/federation-university/GhidraMCP.git
cd GhidraMCP
...
3. Click the `+` button
4. Select the `GhidraMCP-[version].zip` file
5. Restart Ghidra
...
GhidraMCP follows semantic versioning (SemVer) and uses explicit API versions:

### Version Numbers

When making changes, update version numbers in these locations:

1. **Plugin Version** in `src/main/java/au/federation/ghidra/api/ApiConstants.java`:
   ```java
...
1. **API Version** in `src/main/java/au/federation/ghidra/api/ApiConstants.java`:
   ```java
