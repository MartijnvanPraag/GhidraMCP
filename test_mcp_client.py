 #!/usr/bin/env python3
"""
Test script for the GhidraMCP bridge using the MCP client.
...
GhidraMCP_TEST_HOST = os.getenv('GhidraMCP_TEST_HOST', 'localhost')
GhidraMCP_TEST_PORT = int(os.getenv('GhidraMCP_TEST_PORT', '8192'))
...
async def test_bridge():
    ...
    async with stdio_client(server_parameters) as (read_stream, write_stream):
        ...
        async with ClientSession(read_stream, write_stream) as session:
            ...
            logger.info(f"Setting current instance to port {GhidraMCP_TEST_PORT}...")
            use_instance_result = await session.call_tool("instances_use", arguments={"port": GhidraMCP_TEST_PORT})
