...
GhidraMCP is a powerful bridge between [Ghidra](https://ghidra-sre.org/) and AI assistants that enables comprehensive AI-assisted reverse engineering through the [Model Context Protocol (MCP)](https://github.com/modelcontextprotocol/mcp).

![GhidraMCP logo](https://github.com/user-attachments/assets/86b9b2de-767c-4ed5-b082-510b8109f00f)

## Overview

GhidraMCP v2.0 integrates three key components:
...
GhidraMCP is based on [GhidraMCP by Laurie Wired](https://github.com/LaurieWired/GhidraMCP/) but has evolved into a comprehensive reverse engineering platform with enhanced multi-instance support, extensive data manipulation capabilities, and a robust HATEOAS-compliant API architecture.

# Features

GhidraMCP version 2.0 provides a comprehensive set of reverse engineering capabilities to AI assistants through its HATEOAS-driven API:
...
3. Click the `+` button
4. Select the `GhidraMCP-2.0.0-beta.1.zip` (or your chosen version) from the downloaded release
5. Restart Ghidra
...
> ```
>
> GhidraMCP now includes auto-discovery of running Ghidra instances, so manually registering each instance is typically not necessary. The MCP bridge will automatically discover and register instances on startup and periodically check for new ones.
...
Below is an example of how an AI assistant might interact with multiple Ghidra instances using GhidraMCP:
...
GhidraMCP uses structured JSON for all communication between the Python bridge and Java plugin. This ensures consistent and reliable data exchange.

## API Architecture

GhidraMCP v2.0 implements a comprehensive HATEOAS-driven REST API that follows hypermedia design principles:
...
GhidraMCP includes comprehensive test suites for both the HTTP API and MCP bridge. See [TESTING.md](TESTING.md) for details on running the tests.
...
This creates:
- `target/GhidraMCP-[version].zip` - The Ghidra plugin only
- `target/GhidraMCP-Complete-[version].zip` - Complete package with plugin and bridge script
